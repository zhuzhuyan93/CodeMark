...
words = df.title.apply(lambda x:x.split('（')[0]).unique().tolist()  

words = [i for i in words if i not in os.listdir()]
print(len(words)) 
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        for word in words: 
            print(word) 
            crawler = Crawler(0.1)
            crawler.start(word, 10, 2, 30)  # 抓取关键词为 “美女”，总数为 1 页，开始页码为 2，每页30张（即总共 2*30=60 张）
            time.sleep(5)
